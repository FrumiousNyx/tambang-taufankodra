...
const config: ImageConfig = {
  ...
  cdnStrategy: (process.env.NEXT_PUBLIC_IMAGE_CDN as ImageOptimizationStrategy) || 'local',
};
...
export function getImageSizes(
  ...
  const sizes: Record<'hero' | 'thumbnail' | 'product' | 'full', string> = {
...
export function preloadImage(
  ...
  if (context === 'hero') {
    ...
    (link as any).imageSrcset = getImageSrcSet(imagePath, [640, 1024, 1440]);
    (link as any).imageSizes = getImageSizes('hero');
  } else {
