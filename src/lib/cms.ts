...
export const cmsService = {
  async getRawContentByType(type: CMSContent['type']) {
  ...
  async getRawContentById(id: string) {
    const { data, error } = await supabase
      .from('cms_content')
      .select('*')
      .eq('id', id)
  ...
  async getRawPageById(id: string) {
    const { data, error } = await supabase
      .from('cms_pages')
      .select('*')
      .eq('id', id)
  ...
  async getPages(language = 'id') {
    ...
    return (data as CMSPage[]).map(item => ({
      ...item,
      ...
      seoTitle: item.metadata?.seo_title?.[language] || item.metadata?.seo_title?.id,
      seoDescription: item.metadata?.seo_description?.[language] || item.metadata?.seo_description?.id,
    })) as any;
  },
  ...
  async getPageBySlug(slug: string, language = 'id') {
    ...
    return {
      ...
      seoTitle: data.metadata?.seo_title?.[language] || data.metadata?.seo_title?.id,
      seoDescription: data.metadata?.seo_description?.[language] || data.metadata?.seo_description?.id,
    } as any;
  },
