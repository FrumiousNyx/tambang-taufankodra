...
const ProductsSection: React.FC = () => {
  ...
  const getProductFeatures = (id: string) => {
    const value = t(`productsSection.items.${id}.features`, { returnObjects: true });
      ...
      <div className="container-enterprise">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {products.map((product, index) => (
            <ScrollReveal key={product.id} delay={index * 0.1}>
              <Link href={withLang(`/produk/${product.slug}`, language)} className="block group">
                <div className="card-enterprise overflow-hidden h-full">
                  ...
                  <div className={`h-48 bg-gradient-to-br ${product.color} relative overflow-hidden`}>
                    <img 
                      src={product.image}
                      alt={t(`productsSection.items.${product.id}.name`)}
                  ...
                  <div className="p-6">
                    <h3 className="font-bold font-semibold text-xl text-foreground mb-2 group-hover:text-accent transition-colors">
                      {t(`productsSection.items.${product.id}.name`)}
                    </h3>
                    <p className="text-muted-foreground text-sm mb-4 leading-relaxed">
                      {t(`productsSection.items.${product.id}.description`)}
                    </p>

                    {/* Features */}
                    <div className="space-y-2 mb-6">
                      {getProductFeatures(product.id).map((feature, i) => (
        ...
        <ScrollReveal delay={0.4}>
          <div className="text-center mt-12">
            <Link href={withLang('/produk', language)}>
