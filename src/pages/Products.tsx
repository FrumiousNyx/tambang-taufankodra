...
const ProductsPage: React.FC<{ slug?: string }> = ({ slug }) => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (error) {
          ...
          setProducts(PRODUCTS); // Fallback if data is empty
        }
      } catch (err) {
        console.warn('Unexpected error, using fallback data:', err);
        setProducts(PRODUCTS); // Fallback on crash
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return (
    <Layout>
      <SEO title={t('products.title')} description={t('products.subtitle')} lang={language as any} />
  ...
  if (slug) {
    ...
    return (
      <Layout>
        <SEO title={detailName} description={t('products.specs')} lang={language as any} />
        <div className="bg-[#0f172a] pt-32 pb-20 text-white">
          <div className="container-enterprise">
            <Link href={withLang('/produk', language)} className="text-accent flex items-center gap-2 mb-8 uppercase text-[10px] font-bold tracking-widest hover:opacity-70 transition-opacity">
              <ArrowLeft size={14} /> 
          ...
          <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
            {products.map((p) => {
              ...
              const desc = language === 'zh'
                ? (p.description_zh || p.description_en || p.description)
                : language === 'en'
              ...
              return (
                <Link
                  key={p.id}
                  href={withLang(`/produk/${p.slug}`, language)}
